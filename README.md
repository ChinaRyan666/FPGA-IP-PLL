...
# 5 总结
本文讲解了如何基于Vivado软件来调用IP核，并详细讲解了PLL实验，最终仿真测试成功。读者如果有兴趣，可以进一步做验证实验，也就是进行**引脚约束**和**上板验证**，最终使用**示波器**观察时钟的波形图。因为考虑到每个人手里板卡不一样，本文未作此部分验证，本文重点在于讲解IP核的调用以及PLL的原理，希望对您有所帮助。

本文内容基于正点原子《领航者ZYNQ之FPGA开发指南》和笔者实际开发内容撰写，有兴趣的朋友可以进一步联系我交流。
